...
class App(QObject):
    ...
    worker_task = pyqtSignal(dict)

    # File opened
    # Handled by:
    #  * register_folder()
    #  * register_recent()
    file_opened = pyqtSignal(str, str)  # File type and filename
    progress = pyqtSignal(int)  # Percentage of progress
    plots_updated = pyqtSignal()
    ...
    object_created = pyqtSignal(object, bool)
    ...
    def version_string(cls):
        return f"{cls.version} {cls.version_quality} [{cls.version_date}]"
    ...
    def __init__(self, user_defaults=True, post_gui=None):
        ...
        App.log.info("FlatCAM v" + App.version_string() + " starting...")
        ...
        ##        self.ui.connectNotify(self.save_geometry);
        ...
        def auto_save_defaults():
            try:
                ...
                QTimer.singleShot(self.defaults["defaults_save_period_ms"], auto_save_defaults)
        ...
        self.dblsidedtool.install(icon=QIcon('share/doubleside16.png'))

        self.measeurement_tool = Measurement(self)
        self.measeurement_tool.install(icon=QIcon('share/measure16.png'))
        ...
        self.ui.shell_dock.setFeatures(QDockWidget.DockWidgetFeature.DockWidgetMovable |
    ...
    def message_dialog(self, title, message, kind="info"):
        icon = {"info": QMessageBox.Icon.Information,
                "warning": QMessageBox.Icon.Warning,
                "error": QMessageBox.Icon.Critical}[str(kind)]
        ...
        class AboutDialog(QDialog):
            def __init__(self, parent=None):
                ...
                title = QLabel(
                    ...
                    "Manufacturing.<BR>"
                    "<BR>"
                    "(c) 2014-2015 Juan Pablo Caram<br><br>"
                    "Development version from 2022 with support for Qt6" % (App.version_string())
                )
    ...
    def _openFile(self, caption, fileFilter, openFn):
        ...
        try:
            ...
            filename, _= QFileDialog.getOpenFileName(caption=caption, filter=fileFilter)
        filename = str(filename)

        if filename == "":
            self.inform.emit("Open cancelled.")
        else:
            self.worker_task.emit({'fcn': openFn, 'params': [filename]})
    ...
    def on_fileopengerber(self):
        ...
        self._openFile("Open Gerber File", "Gerber Files (*.gbr *.ger *.gtl *.gbl *.gts *.gbs *.gtp *.gbp *.gto *.gbo *.gm1 *.gml *.gm3 *.gko " \
                   ...
                   "Allegro Files (*.art);;" \
                   "Mentor Files (*.pho *.gdo))", self.open_gerber)
    ...
    def on_fileopenexcellon(self):
        ...
        self._openFile("Open Excellon Drill File", 
            "Excellon Files (*.drl *.txt *.xln *.drd *.tap *.exc)",
            self.open_excellon)
    ...
    def on_fileopengcode(self):
        ...
        self._openFile("Open GCode File", "G-Code Files (*.txt *.nc *.ncc *.tap *.gcode *.cnc *.ecs *.fnc *.dnc *.ncg *.gc *.fan *.fgc" \
    ...
    def on_file_openproject(self):
        ...
        try:
            filename, _ = QFileDialog.getOpenFileName(caption="Open Project",
                                                      directory=self.get_last_folder())
        except TypeError:
            filename, _ = QFileDialog.getOpenFileName(caption="Open Project")
    ...
    def on_file_exportsvg(self):
        ...
        if (not isinstance(obj, FlatCAMGeometry) and \
            ...
            not isinstance(obj, FlatCAMCNCjob) and \
    ...
    def on_file_importdxf(self, type_of_obj):
        ...
        try:
            filename, _ = QFileDialog.getOpenFileName(caption="Import DXF",
                                                      directory=self.get_last_folder(), filter=filter)
        except TypeError:
            filename, _ = QFileDialog.getOpenFileName(caption="Import DXF", filter=filter)
    ...
    def final_save(self):
        if self.collection.get_list():
            ...
            msgbox.setWindowIcon(QIcon('share/save_as.png'))
            msgbox.setStandardButtons(QMessageBox.StandardButton.Cancel | QMessageBox.StandardButton.Ok)
            msgbox.setDefaultButton(QMessageBox.StandardButton.Ok)
    ...
    def setup_shell(self):
        ...
        def wait_signal(signal, timeout=10000):
            ...
            if timeout is not None:
                QTimer.singleShot(timeout, report_quit)
    ...
    def version_check(self):
        ...
        :return: None
        """
        self.inform.emit(f"Version {App.version_string()}")
        self.log.debug(f"version_check()")
